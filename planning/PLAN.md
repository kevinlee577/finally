...
- ~~**`.env.example` is referenced as committed but doesn't exist in the repo yet.**~~ **Resolved (2026-08-02):** `.env.example` created at the repo root with the §5 placeholders.
- ~~**`.gitignore` doesn't actually cover `db/finally.db`.**~~ **Resolved (2026-08-02):** added `/db/*.db`, `-wal`, `-shm`, and `-journal` patterns to `.gitignore`.

### Questions / Clarifications

- ~~**Stale prices for positions dropped from the watchlist.**~~ **Resolved (§6):** held positions are always tracked regardless of watchlist membership. `DELETE /api/watchlist/{ticker}` deletes only the `watchlist` row (never calls `source.remove_ticker()`) when a position is open; `POST /api/portfolio/trade` calls `source.remove_ticker()` when a trade zeroes a position that's no longer watchlisted.
